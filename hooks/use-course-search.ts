...
interface UseCourseSearchReturn {
  ...
  resetFilters: () => void;
  loadMore: () => void;
  searchCourses: (filters?: Partial<SearchFilters>, offset?: number) => Promise<void>;
}
...
/* --------------------------------------------
   ...
   Ensures ratings NEVER disappear
--------------------------------------------- */
...
export function useCourseSearch(
  ...
  const [filters, setFilters] = useState<SearchFilters>(defaultFilters);
  const [pagination, setPagination] = useState({
    total: initialCourses.length || 0,
    limit: 12,
    offset: 0,
    hasMore: initialCourses.length > 12,
  });
  ...
  const buildSearchParams = useCallback(
    (filters: SearchFilters, offset: number = 0) => {
      const params = new URLSearchParams();
  ...
  /* --------------------------------------------
     SEARCH COURSES (FIXED)
  --------------------------------------------- */
  const searchCourses = useCallback(
    async (newFilters?: Partial<SearchFilters>, offset: number = 0) => {
      try {
        setLoading(true);
        setError(null);
  ...
  /* --------------------------------------------
     INITIAL COURSES SYNC (FIXED)
  --------------------------------------------- */
  useEffect(() => {
    if (initialCourses.length > 0 && courses.length === 0) {
      const normalized = normalizeCourses(initialCourses);

      setCourses(normalized);
      setPagination(prev => ({
        ...prev,
        total: normalized.length,
        hasMore: normalized.length > prev.limit,
      }));
    }
  }, [initialCourses, courses.length]);
