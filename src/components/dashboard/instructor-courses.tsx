...
export default function InstructorCourses({ courses }: InstructorCoursesProps) {
  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-gray-200">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {courses.map((course) => {
          // Check if course has any assessments/quizzes
          const hasAssessments = (course.assessments_count || course.total_quizzes || 0) > 0;
          ...
          return (
            <div key={course.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                <span>{course.students_count || 0} students</span>
                <span>{course.modules_count || 0} modules</span>
                <span>{course.assessments_count || course.total_quizzes || 0} quizzes</span>
              </div>
              ...
              <div className="flex space-x-2 mt-4">
                ...
                {/* Analytics Button - Only show if course has assessments */}
                {hasAssessments && (
                  <Link 
                    href={`/dashboard/instructor/quizzes?course=${course.id}`}
                    className="flex-1"
                  >
                    <button className="w-full border border-purple-300 text-purple-700 px-3 py-2 rounded text-sm font-medium hover:bg-purple-50 flex items-center justify-center gap-2">
              ...
              {/* Quick stats for quizzes in this course */}
              {hasAssessments && (
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-600">
                    ...
                    <div className="flex items-center gap-4">
                      <span className="flex items-center gap-1">
                        ...
                        Avg: {course.average_quiz_score || 0}%
                      </span>
                      <span className="flex items-center gap-1">
                        <div className="h-2 w-2 rounded-full bg-blue-500"></div>
                        Pass: {course.students_passed || 0}/{course.students_count || 0}
                      </span>
                    </div>
                  </div>
                </div>
